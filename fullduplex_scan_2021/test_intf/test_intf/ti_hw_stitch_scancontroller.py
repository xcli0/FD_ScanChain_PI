...
class Stitch_scan_controller(TI_ABSTRACT_ScanController):
    ...
    def __init__(self, labjack, pin_rst, pin_phi, pin_phi_bar, pin_update, pin_capture, pin_scan_in, 
            ...
            scan_chain_order_csv="./scanChainOrder.csv"): 
        ...
        self.masterScanList=[]
        self.masterScanDict={}
        #self.masterscanDictKeySorted = []
        # decoding CSV file to find all the sequences and settings
        scanComplexDict=testScanModule.readScanCsvFile(scan_chain_order_csv)
        
        # reading scanSequence.txt and scanSettings.txt according to the order
        for index in sorted(scanComplexDict.keys(),reverse=True):
            scanDict = {}
            scanList = []
            #print scanDict
            #print scanList
            print scanComplexDict[index]["sequenceFile"], scanComplexDict[index]["settingsFile"]

            testScanModule.readScanSequenceFile(scanList, scanDict, self.scanDictKeysSorted, scanComplexDict[index]["sequenceFile"])
            testScanModule.readScanSettingsFile(scanDict, scanComplexDict[index]["settingsFile"])
            moduleName = scanComplexDict[index]["module"]
            
            # Now append the scanFiles, ScanList and scanDict into one large entity to feed the taskFile
            # appendScanFiles(module.sequenceFile,module.settingsFile)
            hierScanDict=testScanModule.convertDictKeyToHierName(scanDict,moduleName)
            hierScanList=testScanModule.convertListToHierName(scanList,moduleName)
            hierScanList.reverse()
            self.masterScanList+=hierScanList
            self.masterScanDict.update(hierScanDict)
            #self.masterscanDictKeySorted+= self.scanDictKeysSorted
        # input and output vectors for scanning and comparison
         
        self.inScanVector = testScanModule.genTestScanVector(self.masterScanList, self.masterScanDict) #Generate scanVector needed for scan in
        #print self.scanDictKeysSorted
        #print masterScanList
        #print self.inScanVector[::-1]
        self.outScanVector = []
    ...
    def UpdateStitchScanSettings(self, updateScanSettingsFile):
        testScanModule.ReadupdateScanSettings(self.masterScanDict, updateScanSettingsFile)
        self.inScanVector = testScanModule.genTestScanVector(self.masterScanList, self.masterScanDict)
    ...
    def scan_compare(self):
        ...
        self.outScanDict = testScanModule.genOutputScanDict(self.masterScanList, self.masterScanDict, self.outScanVector)

        #check what is going on
        testScanModule.scanCheck(self.masterScanDict, self.outScanDict, self.scanDictKeysSorted, "scanCompare.txt")
