...
def clean_line(line):
    line=line.strip()
    #if re.search('^\s*#', line): return line  #Ignore opening comments
    line=re.sub('^\s','',line)
    line=re.sub('\s$','',line)
...
#        print "Entering scanVector: " ,name, numBits, direction, value
        # numBits = scanList[i][1]
        # direction = scanList[i][2]
        # value = scanList[i][3]
        #print name, numBits, value
        if(re.match(r'.*[hbd]',value)):
            match=re.match(r'.*(?P<format>[hbd])(?P<val>\w+)',value)
            mFormat=match.group('format')
