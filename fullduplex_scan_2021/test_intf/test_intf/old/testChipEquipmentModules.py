...
class daq_port():
    def __init__(self,port,channel,name,direction,state):
        self.port = port
        self.channel = channel
        self.name = name
...
class daq_usb_6501():
    ...
    def initChannels(self):
       for name in self.nameNumDict:
          port = self.nameNumDict[name][0]
          channel = self.nameNumDict[name][1]
          task_handle = self.nameNumDict[name][2]
          DAQmxCreateTask("",byref(task_handle))
          if self.portList[3] == "output":
    ...
    def readConfigFile(self):
        ...
        for line in lineList:
            ...
            port, channel, name, direction = line
            # port=daq_port(number,name,direction,0)
            # self.portList.append(port)
            # self.portList.append(daq_port(number,name,direction,0))
            self.portList.append(daq_port(port,channel,name,direction,0))
            self.nameNumDict[name]=(port, channel, TaskHandle())

    ##Done reading config file

    def readPortNo(self,port,channel):
        ...
        DAQmxWriteDigitalLines(task_handle, numSamples, 10.0, False, numSamples, None, byref(read), byref(samplesRead), byref(bytesPerSample))
    ...
    def readPortName(self,name):
        ...
        port, channel=int(self.nameNumDict[name])
        logicState=int(self.readPortNo(port, channel))
        print "reading port {0}".format(name)
        return logicState


    def writePortNo(self,port,channel,task_handle,state):
        port=int(port)
        channel=int(channel)
        number = port * 8 + channel
        if(self.portList[number].direction == "out"):
            ...
            DAQmxStopTask(task_handle)
            print "writing {0} to port {1}, channel {2}".format(state, port, channel)
            ########################################################################
            ########################################################################
           if not self.readPortNo(number) == state:
                logging.warning("Writing {0} to port {1}, channel {2} failed which still has state {3}".format(state,port,channel,self.readPortNo(number)))
            else:
    ...
    def writePortName(self,name,state):
        port, channel, task_handle=self.nameNumDict[name]
        logicState=self.writePortNo(port,channel,task_handle,state)
