...
def initializeDaq(portMapFile):
    ...
    ## PLL Specific Signals ##
    daq.writePortName("mgmt_write", 0)
    daq.writePortName("mgmt_read", 0)
    daq.writePortName("mgmt_data_sel_0", 0)
    daq.writePortName("mgmt_data_sel_1", 0)
    daq.writePortName("mgmt_addr_sel_0", 0)
    daq.writePortName("mgmt_addr_sel_1", 0)
    return daq
...
#Pick out the right output print format.
        ...
        # if(rangeMin==rangeMax):
           # outputValue = str(numBits) + "'" + mFormat + str(scanVector[rangeMin])
        # else:
...
def recallFrequency(frequency,fTableFile):
    ...
    with open(fTableFile,'r') as rfh:
        ...
        for f_candidate in fDict.keys():
            if frequency>=fDict[fCandidate]*(2**dividerIndex):
                return_fid = '1'*dividerIndex + fDict[fCandidate]["f_id"]
